(function (exports) {
    ...
    ***************************************************************************** */
    ...
    var fpPromise = index.load();
    var getFingerprintId = async function getFingerprintId() {
    ...
    var fingerprintIdToHash = function fingerprintIdToHash(str) {
      return (// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        str.match(/([a-z0-9]){4}/g).join('-').toUpperCase()
      );
    };

    var genUMID = async function genUMID() {
      try {
        var fingerprintId = await getFingerprintId();
